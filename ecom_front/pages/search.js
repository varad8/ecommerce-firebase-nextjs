...
export default function SearchPage() {
  ...
  useEffect(() => {
    if (productname) {
      axios.get("/api/search?productname=" + productname).then((response) => {
  ...
  return (
    <>
      ...
      <div className="container min-h-screen w-full mx-auto">
        ...
        {!productData?.length && (
          <>
            <div className="flex justify-center  gap-4 flex-wrap items-center mt-5">
              <p className="text-xl text-center">Not Found That Product</p>
              <Link
                href="/products"
                className="px-5 py-3 hover:bg-indigo-600 hover:text-white border bg-white shadow-md rounded-lg"
              >
                See all products
              </Link>
            </div>
